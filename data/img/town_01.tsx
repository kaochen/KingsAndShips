<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.2" tiledversion="1.2.0" name="town_1" tilewidth="256" tileheight="256" tilecount="1" columns="1">
 <tileoffset x="-70" y="55"/>
 <grid orientation="orthogonal" width="8" height="8"/>
 <properties>
  <property name="unit.type" type="str" value="town"/>
  <property name="unit.health" type="int" value="100"/>
  <property name="unit.cost" type="int" value="500"/>
  <property name="unit.rank" type="int" value="1"/>
  <property name="unit.speed" type="int" value="0"/>
  <property name="weapon.type" type="str" value="archerTower"/>
  <property name="weapon.damage" type="int" value="3"/>
  <property name="weapon.firerange" type="int" value="2"/>
  <property name="weapon.firerate" type="int" value="2000"/>
  <property name="weapon.speedImpact" type="int" value="0"/>
 </properties>
 <image source="original/Town_01.png" width="256" height="256"/>
 <tile id="0" type="A_EE_0"/>
</tileset>
